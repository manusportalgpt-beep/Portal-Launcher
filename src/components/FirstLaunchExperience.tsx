...
export function FirstLaunchExperience() {
  ...
  useEffect(() => { const openTutorial = () => setMode('tutorial'); window.addEventListener('portal:open-tutorial', openTutorial); return () => window.removeEventListener('portal:open-tutorial', openTutorial); }, []);
  ...
  return <AnimatePresence><motion.div className="fixed inset-0 z-[230] grid place-items-center overflow-y-auto bg-black p-5" style={{ color:'var(--color-text)' }} initial={{ opacity:0 }} animate={{ opacity:1 }}><motion.section className="my-auto w-full max-w-2xl" style={{ background:'var(--color-bg)', border:'1px solid var(--color-border)', borderRadius:2 }} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}><header className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor:'var(--color-border)' }}><div><p className="text-sm font-bold">Первый запуск</p><p className="mt-0.5 text-xs" style={{ color:'var(--color-text-secondary)' }}>{steps[step]}</p></div><span className="text-xs" style={{ color:'var(--color-text-tertiary)' }}>{step + 1} / {steps.length}</span></header><main className="p-5 sm:p-6">
    ...
    {step === 3 && <div className="mx-auto max-w-md"><h1 className="text-2xl font-bold">Аккаунт</h1><p className="mt-2 text-sm" style={{ color:'var(--color-text-secondary)' }}>Microsoft нужен для лицензионного Minecraft. Вход можно пропустить и добавить аккаунт позже в настройках.</p><div className="mt-5 p-3" style={{ background:'transparent', border:'1px solid var(--color-border)', borderRadius:2 }}><MicrosoftAuthOAuth onSuccess={finishSetup} /></div></div>}
