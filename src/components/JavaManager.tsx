...
export function JavaManager({ selectedPath, onSelect }: { selectedPath: string; onSelect: (path: string) => void }) {
  ...
  const installRecommended = async (version: number) => {
  ...
  return <section className="mt-5 overflow-hidden rounded-2xl" style={{ border:'1px solid var(--color-border)', background:'color-mix(in srgb, var(--color-surface) 96%, transparent)' }}>
    ...
    <div className="space-y-4 p-4">
      {RUNTIME_VERSIONS.map(version => {
        ...
        const isSelected = Boolean(runtime && selectedPath === runtime.path);
        const isBusy = busyVersion === version;
        const isDownloading = isBusy && progress?.version === version;
        return <article key={version} className="rounded-2xl p-3.5" style={{ background:'var(--color-surface-2)', border:`1px solid ${isSelected ? 'var(--color-primary)' : 'var(--color-border)'}` }}>
          <div className="flex flex-wrap items-start justify-between gap-2"><div><div className="flex items-center gap-2"><h4 className="text-sm font-black" style={{ color:'var(--color-text)' }}>Java {version}</h4>{runtime ? <span className="inline-flex items-center gap-1 text-[10px] font-bold" style={{ color:'var(--color-success, var(--color-primary))' }}><CheckCircle2 className="h-3.5 w-3.5" />Готова</span> : <span className="inline-flex items-center gap-1 text-[10px] font-bold" style={{ color:'var(--color-text-tertiary)' }}><XCircle className="h-3.5 w-3.5" />Не установлена</span>}</div><p className="mt-1 text-[10px]" style={{ color:'var(--color-text-secondary)' }}>{runtimePurpose(version)}</p></div>{isSelected && <span className="rounded-full px-2 py-1 text-[9px] font-black" style={{ background:'var(--color-primary-dim)', color:'var(--color-primary)' }}>ВЫБРАНА</span>}</div>
          ...
          <div className="mt-2.5 flex flex-wrap gap-2"><button type="button" disabled={busyVersion !== null} onClick={() => void installRecommended(version)} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[11px] font-black disabled:cursor-wait disabled:opacity-50" style={{ background:'var(--color-primary)', color:'var(--color-primary-text)' }}>{isDownloading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}{isDownloading ? 'Устанавливаю…' : 'Установить рекомендуемую'}</button><button type="button" disabled={busyVersion !== null} onClick={() => void detect(version)} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[11px] font-bold disabled:opacity-50" style={{ background:'var(--color-surface)', color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}><Search className="h-3.5 w-3.5" />Найти</button><button type="button" disabled={busyVersion !== null} onClick={() => void browse(version)} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[11px] font-bold disabled:opacity-50" style={{ background:'var(--color-surface)', color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}><FolderOpen className="h-3.5 w-3.5" />Выбрать файл</button></div>
