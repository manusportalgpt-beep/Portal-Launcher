...
export function ScreenshotEditor({ instanceId, fileName, imageUrl, onClose, onSaved }: ScreenshotEditorProps) {
  ...
  return createPortal(<div data-portal-overlay="true" className="fixed inset-0 flex flex-col bg-[#09090b]/95 backdrop-blur-md" style={{ zIndex: 2147483647 }}>
    <div className="flex items-center gap-3 border-b px-4 py-3" style={{ borderColor:'var(--color-border)' }}><Paintbrush className="h-4 w-4" style={{ color:'var(--color-primary)' }} /><p className="min-w-0 flex-1 truncate text-sm font-bold" style={{ color:'var(--color-text)' }}>{fileName}{dirty ? t('instanceUi.editor.modified') : ''}</p><button onClick={undo} disabled={!currentHistory || currentHistory.index <= 0} className="rounded-lg p-2 disabled:opacity-30" title="Отменить"><Undo2 className="h-4 w-4" /></button><button onClick={redo} disabled={!currentHistory || currentHistory.index >= currentHistory.items.length - 1} className="rounded-lg p-2 disabled:opacity-30" title="Повторить"><Redo2 className="h-4 w-4" /></button><button onClick={save} disabled={saving || !dirty} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold disabled:opacity-40" style={{ background:'var(--color-primary)', color:'var(--color-primary-text)' }}><Save className="h-3.5 w-3.5" />{saving ? t('instanceUi.editor.saving') : t('common.save')}</button><button data-portal-close="true" onClick={onClose} className="rounded-lg p-2" title={t('common.close')}><X className="h-4 w-4" /></button></div>
    <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden p-6"><div className="relative rounded-xl p-3 shadow-2xl" style={{ background:'repeating-conic-gradient(#242424 0% 25%, #1b1b1b 0% 50%) 50% / 24px 24px' }}><canvas ref={baseCanvasRef} className="block max-h-[calc(100vh-290px)] max-w-[calc(100vw-48px)] object-contain shadow-xl" style={{ filter:imageFilter }} />{layers.map(layer => <canvas key={layer.id} ref={element => { layerCanvasRefs.current[layer.id] = element; }} onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerCancel={end} className="absolute left-3 top-3 max-h-[calc(100vh-290px)] max-w-[calc(100vw-48px)] touch-none object-contain" style={{ display:layer.visible ? 'block' : 'none', pointerEvents:layer.id === activeLayer && layer.visible ? 'auto' : 'none' }} />)}</div></div>
    <div className="flex flex-wrap items-center justify-center gap-2 border-t px-4 py-3" style={{ borderColor:'var(--color-border)', background:'var(--color-surface)' }}>
      <div className="flex max-w-full items-center gap-1 rounded-xl px-2 py-1.5" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)' }}><Layers3 className="h-3.5 w-3.5" style={{ color:'var(--color-primary)' }} />{layers.map(layer => <div key={layer.id} className="flex items-center"><button onClick={() => setActiveLayer(layer.id)} className="rounded-lg px-1.5 py-1 text-[10px] font-bold" style={{ background:activeLayer === layer.id ? 'var(--color-primary-dim)' : 'transparent', color:activeLayer === layer.id ? 'var(--color-primary)' : 'var(--color-text-secondary)' }}>{layer.name}</button><button onClick={() => setLayers(current => current.map(item => item.id === layer.id ? { ...item, visible: !item.visible } : item))} className="p-1" title={layer.visible ? 'Скрыть слой' : 'Показать слой'}>{layer.visible ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}</button>{layers.length > 1 && <button onClick={() => removeLayer(layer.id)} className="p-1" title="Удалить слой"><Trash2 className="h-3 w-3" /></button>}</div>)}<button onClick={addLayer} className="rounded-lg p-1" title="Создать слой"><Plus className="h-3.5 w-3.5" /></button></div>
