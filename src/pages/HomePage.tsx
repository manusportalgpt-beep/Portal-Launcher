        ...
        <div className="relative grid gap-7 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-end">
          <div className="max-w-3xl">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <button type="button" onClick={() => navigate(signedIn ? '/library' : '/settings/account')} className="inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-black transition-colors" style={{ background: 'var(--color-primary)', color: 'var(--color-primary-text)', boxShadow: 'none' }}>{signedIn ? <><Play className="h-4 w-4 fill-current" />Мои сборки</> : <><ShieldCheck className="h-4 w-4" />Войти в аккаунт</>}</button>
              <button type="button" onClick={() => navigate('/library?create=1')} className="inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-bold transition-colors" style={{ background: 'transparent', color: 'var(--color-text)', border: '1px solid var(--color-border)' }}><FolderPlus className="h-4 w-4" />Новая сборка</button>
