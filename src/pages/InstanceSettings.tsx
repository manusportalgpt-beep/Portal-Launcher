...
const tabs = [
  { id:'general', label:'Основное', icon:Settings2, desc:'Название, версия и загрузчик' },
  { id:'java', label:'Java и память', icon:Cpu, desc:'Автоматический выбор среды' },
  { id:'content', label:'Моды', icon:Package, desc:'Моды, ресурспаки и шейдеры' },
  { id:'maintenance', label:'Обслуживание', icon:Wrench, desc:'Резервные копии и безопасный режим' },
];
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)' }}>
      <div className="flex items-start justify-between gap-3">
        <div><div className="flex items-center gap-2"><History className="w-4 h-4" style={{ color:'var(--color-primary)' }} /><p className="text-sm font-black" style={{ color:'var(--color-text)' }}>Update rollback history</p></div><p className="mt-1 text-xs" style={{ color:'var(--color-text-secondary)' }}>Every successful automatic Modrinth update now saves its previous files. Restore one mod or the complete update batch by date.</p></div>
        <button onClick={() => void loadSnapshots()} className="h-8 w-8 shrink-0 rounded-lg flex items-center justify-center hover:bg-white/5" title="Refresh update history" style={{ color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}><RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /></button>
      </div>
      {error && <p className="mt-3 rounded-xl px-3 py-2 text-xs" style={{ background:'rgba(231,76,60,0.1)', color:'var(--color-error)' }}>{error}</p>}
      {!loading && snapshots.length === 0 && <p className="mt-3 rounded-xl border border-dashed px-3 py-4 text-center text-xs" style={{ borderColor:'var(--color-border)', color:'var(--color-text-tertiary)' }}>No update snapshots yet. The launcher will create one before the next successful mod update.</p>}
      <div className="mt-3 space-y-2">
        {snapshots.map(snapshot => (
          <div key={snapshot.id} className="rounded-xl p-3" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)' }}>
            <div className="flex items-center justify-between gap-2"><div><p className="text-xs font-black" style={{ color:'var(--color-text)' }}>{new Date(snapshot.timestamp).toLocaleString()}</p><p className="text-[10px]" style={{ color:'var(--color-text-tertiary)' }}>{snapshot.entries.length} update{snapshot.entries.length === 1 ? '' : 's'} captured before installation</p></div><button onClick={() => void restore(snapshot)} disabled={busy !== null} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-black disabled:opacity-50" style={{ background:'var(--color-primary-dim)', color:'var(--color-primary)', border:'1px solid var(--color-primary)' }}>{busy === `${snapshot.id}:all` ? <RefreshCw className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}Restore all</button></div>
            <div className="mt-2 space-y-1">
              {snapshot.entries.map(entry => <div key={entry.mod_id} className="flex items-center gap-2 rounded-lg px-2 py-1.5" style={{ background:'var(--color-surface-2)' }}><span className="min-w-0 flex-1 truncate text-[11px] font-semibold" style={{ color:'var(--color-text)' }}>{entry.mod_name}</span><span className="text-[10px]" style={{ color:'var(--color-text-tertiary)' }}>{entry.updated.version} → {entry.previous.version}</span><button onClick={() => void restore(snapshot, entry.mod_id)} disabled={busy !== null} className="rounded-md px-2 py-1 text-[9px] font-black disabled:opacity-50" style={{ color:'var(--color-primary)', border:'1px solid var(--color-border)' }}>Restore mod</button></div>)}
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-8"
      ...
      <motion.div initial={{ opacity:0, y:16, scale:0.97 }} animate={{ opacity:1, y:0, scale:1 }} transition={{ type:'spring', stiffness:420, damping:32 }}
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-3.5 shrink-0" style={{ background:'color-mix(in srgb, var(--color-surface) 94%, transparent)', borderBottom:'1px solid var(--color-border)' }}>
          <div className="flex min-w-0 items-center gap-2 text-sm"><button onClick={()=>navigate(-1)} className="flex items-center gap-1.5 hover:opacity-80" style={{ color:'var(--color-text-secondary)' }}><ChevronLeft className="h-4 w-4" />Назад к библиотеке</button><span style={{ color:'var(--color-text-tertiary)' }}>/</span><span className="truncate font-semibold" style={{ color:'var(--color-text)' }}>{form.name || inst.name}</span></div>
          <div className="flex shrink-0 items-center gap-2"><button onClick={() => navigate(`/library/${inst.id}`)} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text-secondary)' }}><Play className="h-3.5 w-3.5 fill-current" />Играть</button><button onClick={handleSave} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all" style={{ background:saved?'rgba(46,204,113,0.15)':'var(--color-primary)', color:saved?'var(--color-success)':'var(--color-primary-text)', border:saved?'1px solid var(--color-success)':'1px solid var(--color-primary)' }}><Save className="h-3.5 w-3.5" />{saved?'Сохранено':'Сохранить'}</button></div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 gap-4 p-4 sm:p-5">
          <aside className="flex w-52 shrink-0 flex-col gap-1.5 p-2.5" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-card)' }}>
            {tabs.map(({id:tabId,label,icon:Icon,desc}) => <button key={tabId} onClick={() => setTab(tabId)} className="flex items-center gap-2.5 px-2.5 py-2.5 text-left transition-all" style={{ borderRadius:'var(--radius-button)', background:tab===tabId?'var(--color-primary-dim)':'transparent', color:tab===tabId?'var(--color-text)':'var(--color-text-secondary)', border:`1px solid ${tab===tabId?'var(--color-primary)':'transparent'}` }}><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ background:tab===tabId?'var(--color-primary)':'var(--color-surface-2)', color:tab===tabId?'var(--color-primary-text)':'var(--color-text-secondary)' }}><Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-xs font-black">{label}</span><span className="block truncate text-[9px]" style={{ color:'var(--color-text-tertiary)' }}>{desc}</span></span></button>)}
            <div className="mt-auto p-3" style={{ background:'var(--color-surface-2)', borderRadius:'var(--radius-button)', border:'1px solid var(--color-border)' }}><div className="flex items-center gap-1.5 text-[10px] font-black" style={{ color:'var(--color-primary)' }}><ShieldCheck className="h-3.5 w-3.5" />БЕЗОПАСНЫЕ ИЗМЕНЕНИЯ</div><p className="mt-1 text-[9px] leading-3" style={{ color:'var(--color-text-secondary)' }}>Резервные копии и безопасный режим защищают сборку во время проверки изменений.</p></div>
          </aside>
            ...
            <div className="space-y-6 p-5 sm:p-6">
              {tab==='general' && <>
                <section className="relative overflow-hidden p-4 sm:p-5" style={{ background:'linear-gradient(135deg, color-mix(in srgb, var(--color-primary) 14%, var(--color-surface)), var(--color-surface-2))', border:'1px solid var(--color-border)', borderRadius:'var(--radius-card)' }}>
                  <div className="absolute -right-14 -top-14 h-48 w-48 rounded-full" style={{ background:'radial-gradient(circle, var(--color-primary-dim), transparent 70%)' }} />
                  <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center"><input ref={coverInputRef} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={event => pickCover(event.target.files?.[0])} /><button onClick={() => coverInputRef.current?.click()} className="group relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden" style={{ background:'var(--color-surface)', border:'2px dashed var(--color-border)', borderRadius:'var(--radius-card)' }}>{coverSrc ? <img src={coverSrc} alt="Instance cover" className="h-full w-full object-cover" /> : <Package className="h-8 w-8" style={{ color:'var(--color-text-tertiary)' }} />}<span className="absolute inset-0 flex items-center justify-center bg-black/55 opacity-0 transition-opacity group-hover:opacity-100"><ImagePlus className="h-5 w-5 text-white" /></span></button><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[0.16em]" style={{ color:'var(--color-primary)' }}>Instance profile</p><h1 className="mt-1 truncate text-xl font-black" style={{ color:'var(--color-text)' }}>{form.name || inst.name}</h1><div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-lg px-2 py-1 text-[10px] font-bold" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', color:'var(--color-text-secondary)' }}>Minecraft {form.minecraftVersion}</span><span className="rounded-lg px-2 py-1 text-[10px] font-bold capitalize" style={{ background:'var(--color-primary-dim)', border:'1px solid var(--color-primary)', color:'var(--color-primary)' }}>{activeLoader.label}{form.modLoaderVersion ? ` ${form.modLoaderVersion}` : ''}</span></div><div className="mt-3 flex gap-2"><button onClick={() => coverInputRef.current?.click()} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', color:'var(--color-text-secondary)' }}><ImagePlus className="h-3.5 w-3.5" />Change cover</button>{form.iconPath && <button onClick={() => setForm(current => ({ ...current, iconPath:'' }))} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold" style={{ background:'rgba(231,76,60,0.1)', color:'var(--color-error)' }}><Trash2 className="h-3.5 w-3.5" />Remove</button>}</div></div></div>
                </section>

                <div className="grid gap-3 sm:grid-cols-2"><div className="p-3.5" style={{ background:'var(--color-primary-dim)', border:'1px solid var(--color-primary)', borderRadius:'var(--radius-button)' }}><div className="flex items-center gap-2"><Cpu className="h-4 w-4" style={{ color:'var(--color-primary)' }} /><p className="text-xs font-black" style={{ color:'var(--color-text)' }}>Automatic Java {recommendedJava}</p></div><p className="mt-1 text-[10px] leading-relaxed" style={{ color:'var(--color-text-secondary)' }}>{form.javaPath ? 'Custom Java is selected for this instance.' : `Portal Launcher will choose or download compatible Java ${recommendedJava} when the instance starts.`}</p></div><div className="p-3.5" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-button)' }}><div className="flex items-center gap-2"><Info className="h-4 w-4" style={{ color:'var(--color-primary)' }} /><p className="text-xs font-black" style={{ color:'var(--color-text)' }}>Ready to customize</p></div><p className="mt-1 text-[10px] leading-relaxed" style={{ color:'var(--color-text-secondary)' }}>Changing the game version or loader preserves your instance profile; create a backup before changing installed content.</p></div></div>
                ...
                <section><div className="mb-2 flex items-center justify-between"><div><p className="text-sm font-black" style={{ color:'var(--color-text)' }}>Game core</p><p className="mt-0.5 text-[10px]" style={{ color:'var(--color-text-secondary)' }}>Choose a loader before selecting its compatible build.</p></div></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{LOADERS.map(loader => { const meta = LOADER_META[loader]; if (!meta) return null; const selected = form.modLoader === loader; return <button key={loader} onClick={() => setForm(current => ({ ...current, modLoader:loader as any, modLoaderVersion:'' }))} className="flex items-center gap-2.5 p-3 text-left transition-all" style={{ background:selected?'var(--color-primary-dim)':'var(--color-surface-2)', border:`1px solid ${selected?'var(--color-primary)':'var(--color-border)'}`, borderRadius:'var(--radius-button)' }}><span className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background:selected?'var(--color-primary)':'var(--color-surface)', color:selected?'var(--color-primary-text)':'var(--color-primary)' }}><meta.Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-xs font-bold" style={{ color:'var(--color-text)' }}>{meta.label}</span><span className="block truncate text-[9px]" style={{ color:'var(--color-text-tertiary)' }}>{meta.description}</span></span>{selected && <Check className="ml-auto h-3.5 w-3.5" style={{ color:'var(--color-primary)' }} />}</button>; })}</div></section>

                <div className="grid gap-5 sm:grid-cols-2"><Field label={t('instances.version')} desc="Release versions are shown by default; enable snapshots only when you need them."><div className="flex gap-2"><select value={form.minecraftVersion} onChange={event => setForm(current => ({ ...current, minecraftVersion:event.target.value, modLoaderVersion:'' }))} className="min-w-0 flex-1 px-3 py-2.5 text-sm font-bold outline-none" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text)', borderRadius:'var(--radius-button)' }}>{(mcVersions.includes(form.minecraftVersion) ? mcVersions : [form.minecraftVersion, ...mcVersions]).map(version => <option key={version} value={version}>{version}</option>)}</select><button onClick={() => setShowSnapshots(value => !value)} className="shrink-0 rounded-xl px-2.5 text-[10px] font-bold" style={{ background:showSnapshots?'var(--color-primary-dim)':'var(--color-surface-2)', border:`1px solid ${showSnapshots?'var(--color-primary)':'var(--color-border)'}`, color:showSnapshots?'var(--color-primary)':'var(--color-text-secondary)' }}>{showSnapshots?'Snapshots':'Releases'}</button></div></Field><Field label="Loader build" desc={loaderVersionsLoading ? 'Looking for compatible builds…' : recommendedLoaderVersion ? `Recommended: ${recommendedLoaderVersion.value}` : 'Choose a compatible build or enter one manually.'}><div className="flex gap-2"><select value={form.modLoaderVersion} onChange={event => setForm(current => ({ ...current, modLoaderVersion:event.target.value }))} disabled={loaderVersionsLoading || loaderVersions.length === 0} className="min-w-0 flex-1 px-3 py-2.5 text-sm outline-none disabled:opacity-60" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text)', borderRadius:'var(--radius-button)' }}><option value="">{loaderVersionsLoading ? 'Loading…' : loaderVersions.length ? 'Automatic / recommended' : 'Manual version'}</option>{loaderVersions.map(version => <option key={version.value} value={version.value}>{version.value}{version.recommended ? ' · Recommended' : version.unstable ? ' · Preview' : ''}</option>)}</select>{recommendedLoaderVersion && <button onClick={() => setForm(current => ({ ...current, modLoaderVersion:recommendedLoaderVersion.value }))} title="Use recommended loader version" className="shrink-0 rounded-xl px-2.5" style={{ background:'var(--color-primary-dim)', border:'1px solid var(--color-primary)', color:'var(--color-primary)' }}><Check className="h-4 w-4" /></button>}</div><input value={form.modLoaderVersion} onChange={event => setForm(current => ({ ...current, modLoaderVersion:event.target.value }))} placeholder={recommendedLoaderVersion?.value || 'Loader version'} className="mt-2 w-full px-3 py-2 text-xs outline-none" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text)', borderRadius:'var(--radius-button)' }} /></Field></div>
              </>}
              ...
              {tab==='content' && <div className="flex h-56 flex-col items-center justify-center gap-3"><Folder className="h-9 w-9" style={{color:'var(--color-primary)'}} /><div className="text-center"><p className="text-sm font-black" style={{color:'var(--color-text)'}}>Управление модами сборки</p><p className="mt-1 text-xs" style={{color:'var(--color-text-secondary)'}}>Устанавливайте моды, шейдеры и ресурспаки или откройте редактор файлов на странице сборки.</p></div><button onClick={() => navigate(`/library/${inst.id}`)} className="rounded-xl px-4 py-2 text-xs font-black" style={{ background:'var(--color-primary)', color:'var(--color-primary-text)' }}>Открыть сборку</button></div>}
