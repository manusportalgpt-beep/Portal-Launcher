...
export function InstanceSettings() {
  ...
  // Значения памяти из общих настроек лаунчера (Настройки → Minecraft).
  // Используются для предзаполнения полей сборки — не меняют логику запуска.
  const launcherMinRam = useSettingsStore(s => s.minRam);
  const launcherMaxRam = useSettingsStore(s => s.maxRam);
  ...
  const [form, setForm] = useState({
    ...
    minRam: (inst.minRam === 0 || inst.minRam === 1024) ? launcherMinRam : inst.minRam,
    maxRam: (inst.maxRam === 0 || inst.maxRam === 4096) ? launcherMaxRam : inst.maxRam,
  });
  ...
  const handleSave = async () => {
    const versionChanged = form.minecraftVersion !== inst.minecraftVersion || form.modLoader !== inst.modLoader || (form.modLoaderVersion || '') !== (inst.modLoaderVersion || '');
    if (versionChanged) {
      const approved = await dialog.confirm(
        `Смена версии или загрузчика изменит профиль сборки:\n\nMinecraft: ${inst.minecraftVersion} → ${form.minecraftVersion}\nЗагрузчик: ${inst.modLoader}${form.modLoaderVersion ? ` (${form.modLoaderVersion})` : ''}\n\nРекомендуется создать резервную копию перед сменой ядра.\n\nПродолжить?`,
        { title: 'Смена версии сборки', confirmLabel: 'Сменить версию', cancelLabel: 'Отмена' }
      );
      if (!approved) return;
    }
          ...
          <motion.main key={tab} initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} className="min-w-0 flex-1 overflow-y-auto scroll-area" style={{ background:'var(--color-bg)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-card)' }}>
            <div className="space-y-6 p-5 sm:p-6">
              {tab==='general' && <>
                ...
                <div className="grid gap-5 sm:grid-cols-2"><Field label={t('instances.version')} desc="По умолчанию показаны релизы. Включайте снимки только при необходимости."><div className="flex gap-2"><VersionPicker versions={mcVersions} value={form.minecraftVersion} onChange={value => setForm(current => ({ ...current, minecraftVersion:value, modLoaderVersion:'' }))} showSnapshots={showSnapshots} /><button type="button" role="switch" aria-checked={showSnapshots} onClick={() => setShowSnapshots(value => !value)} className="flex shrink-0 items-center gap-2 rounded-full px-2.5 py-1.5 text-[10px] font-bold transition-all" style={{ background:showSnapshots?'var(--color-primary-dim)':'var(--color-surface-2)', border:`1px solid ${showSnapshots?'var(--color-primary)':'var(--color-border)'}`, color:showSnapshots?'var(--color-primary)':'var(--color-text-tertiary)' }}><span className="relative h-3.5 w-6 rounded-full" style={{ background:showSnapshots?'var(--color-primary)':'var(--color-border)' }}><span className="absolute top-0.5 h-2.5 w-2.5 rounded-full bg-white transition-all" style={{ left:showSnapshots?11:2 }} /></span>Snapshot-версии</button></div></Field><Field label="Сборка загрузчика" desc={loaderVersionsLoading ? 'Ищу совместимые сборки…' : recommendedLoaderVersion ? `Рекомендуемая: ${recommendedLoaderVersion.value}` : 'Выберите совместимую сборку или укажите её вручную.'}><div className="flex gap-2"><select value={form.modLoaderVersion} onChange={event => setForm(current => ({ ...current, modLoaderVersion:event.target.value }))} disabled={loaderVersionsLoading || loaderVersions.length === 0} className="min-w-0 flex-1 px-3 py-2.5 text-sm outline-none disabled:opacity-60" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text)', borderRadius:'var(--radius-button)' }}><option value="">{loaderVersionsLoading ? 'Загрузка…' : loaderVersions.length ? 'Автоматически / рекомендуемая' : 'Версия вручную'}</option>{loaderVersions.map(version => <option key={version.value} value={version.value}>{version.value}{version.recommended ? ' · Рекомендуемая' : version.unstable ? ' · Предпросмотр' : ''}</option>)}</select>{recommendedLoaderVersion && <button onClick={() => setForm(current => ({ ...current, modLoaderVersion:recommendedLoaderVersion.value }))} title="Использовать рекомендуемую сборку" className="shrink-0 rounded-xl px-2.5" style={{ background:'transparent', border:'1px solid var(--color-primary)', color:'var(--color-primary)' }}><Check className="h-4 w-4" /></button>}</div><input value={form.modLoaderVersion} onChange={event => setForm(current => ({ ...current, modLoaderVersion:event.target.value }))} placeholder={recommendedLoaderVersion?.value || 'Версия загрузчика'} className="mt-2 w-full px-3 py-2 text-xs outline-none" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text)', borderRadius:'var(--radius-button)' }} />{(recommendedLoaderVersion || loaderVersions.some(version => version.unstable)) && <div className="mt-2 flex flex-wrap gap-1.5">{recommendedLoaderVersion && <span className="inline-flex items-center gap-1 px-1.5 py-1 rounded-md text-[9px] font-bold" style={{ background:'rgba(46,204,113,0.12)', color:'#2ECC71', border:'1px solid rgba(46,204,113,0.28)' }}><Check className="w-3 h-3" />Рекомендуемая: {recommendedLoaderVersion.value}</span>}{loaderVersions.some(version => version.unstable) && <span className="inline-flex items-center gap-1 px-1.5 py-1 rounded-md text-[9px] font-bold" style={{ background:'rgba(243,156,18,0.12)', color:'var(--color-warning)', border:'1px solid rgba(243,156,18,0.28)' }}>Возможны нестабильные / preview-версии</span>}</div>}</Field></div>
              </>}
