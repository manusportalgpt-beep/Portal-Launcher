...
const tabs = [
  { id:'general', label:'General', icon:Settings2, desc:'Name, version and loader' },
  { id:'java', label:'Java & Memory', icon:Cpu, desc:'Automatic runtime selection' },
  { id:'content', label:'Content', icon:Package, desc:'Mods, packs and shaders' },
  { id:'maintenance', label:'Maintenance', icon:Wrench, desc:'Backup and Safe Mode' },
];
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-8"
      ...
      <motion.div initial={{ opacity:0, y:16, scale:0.97 }} animate={{ opacity:1, y:0, scale:1 }} transition={{ type:'spring', stiffness:420, damping:32 }}
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-3.5 shrink-0" style={{ background:'color-mix(in srgb, var(--color-surface) 94%, transparent)', borderBottom:'1px solid var(--color-border)' }}>
          <div className="flex min-w-0 items-center gap-2 text-sm"><button onClick={()=>navigate(-1)} className="flex items-center gap-1.5 hover:opacity-80" style={{ color:'var(--color-text-secondary)' }}><ChevronLeft className="h-4 w-4" />Back to Library</button><span style={{ color:'var(--color-text-tertiary)' }}>/</span><span className="truncate font-semibold" style={{ color:'var(--color-text)' }}>{form.name || inst.name}</span></div>
          <div className="flex shrink-0 items-center gap-2"><button onClick={() => navigate(`/library/${inst.id}`)} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold" style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', color:'var(--color-text-secondary)' }}><Play className="h-3.5 w-3.5 fill-current" />Play</button><button onClick={handleSave} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all" style={{ background:saved?'rgba(46,204,113,0.15)':'var(--color-primary)', color:saved?'var(--color-success)':'var(--color-primary-text)', border:saved?'1px solid var(--color-success)':'1px solid var(--color-primary)' }}><Save className="h-3.5 w-3.5" />{saved?'Saved!':'Save Changes'}</button></div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 gap-4 p-4 sm:p-5">
          <aside className="flex w-52 shrink-0 flex-col gap-1.5 p-2.5" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-card)' }}>
            ...
            <div className="mt-auto p-3" style={{ background:'var(--color-surface-2)', borderRadius:'var(--radius-button)', border:'1px solid var(--color-border)' }}><div className="flex items-center gap-1.5 text-[10px] font-black" style={{ color:'var(--color-primary)' }}><ShieldCheck className="h-3.5 w-3.5" />SAFE CHANGES</div><p className="mt-1 text-[9px] leading-3" style={{ color:'var(--color-text-secondary)' }}>Backups and Safe Mode protect this instance while you test changes.</p></div>
          </aside>
          ...
          <motion.main key={tab} initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} className="min-w-0 flex-1 overflow-y-auto scroll-area" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-card)' }}>
            <div className="space-y-6 p-5 sm:p-6">
              ...
              {tab==='content' && <div className="flex h-56 flex-col items-center justify-center gap-3"><Folder className="h-9 w-9" style={{color:'var(--color-primary)'}} /><div className="text-center"><p className="text-sm font-black" style={{color:'var(--color-text)'}}>Manage instance content</p><p className="mt-1 text-xs" style={{color:'var(--color-text-secondary)'}}>Install mods, shaders and resource packs or open the Files editor from the instance page.</p></div><button onClick={() => navigate(`/library/${inst.id}`)} className="rounded-xl px-4 py-2 text-xs font-black" style={{ background:'var(--color-primary)', color:'var(--color-primary-text)' }}>Open instance</button></div>}
