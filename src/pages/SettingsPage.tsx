...
function ControlsSection() {
  ...
  return <div className="max-w-3xl space-y-4">
    ...
    <section className="overflow-hidden rounded-2xl" style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)' }}>
      {(Object.keys(HOTKEY_LABELS) as HotkeyAction[]).map(action => <div key={action} className="flex items-center gap-4 px-4 py-3" style={{ borderBottom:'1px solid var(--color-border)' }}><div className="min-w-0 flex-1"><p className="text-sm font-bold" style={{ color:'var(--color-text)' }}>{HOTKEY_LABELS[action].label}</p><p className="mt-0.5 text-[11px]" style={{ color:'var(--color-text-secondary)' }}>{HOTKEY_LABELS[action].description}</p></div><div className="flex items-center gap-2">{!bindings[action] && <button title="Снять сочетание" onClick={() => setBinding(action, '')} className="rounded-xl px-2 py-2 text-xs font-bold" style={{ background:'var(--color-surface-2)', color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}>Выкл</button>}<button onClick={() => setCapturing(action)} className="min-w-28 rounded-xl px-3 py-2 text-xs font-black" style={{ background:capturing === action ? 'var(--color-primary)' : 'var(--color-surface-2)', color:capturing === action ? 'var(--color-primary-text)' : 'var(--color-text)', border:`1px solid ${capturing === action ? 'var(--color-primary)' : 'var(--color-border)'}` }}>{capturing === action ? 'Нажмите…' : (bindings[action] || 'Не назначено')}</button></div></div>)}
      <div className="flex items-center justify-between gap-3 px-4 py-3"><button onClick={() => { void openBrowserWindow(); }} className="rounded-xl px-3 py-2 text-xs font-bold" style={{ background:'var(--color-surface-2)', color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}>Открыть браузер сейчас</button><span className="text-[11px]" style={{ color:'var(--color-text-tertiary)' }}>Изменения сохраняются автоматически.</span><button onClick={() => reset()} className="rounded-xl px-3 py-2 text-xs font-bold" style={{ background:'var(--color-surface-2)', color:'var(--color-text-secondary)', border:'1px solid var(--color-border)' }}>Сбросить: {HOTKEY_DEFAULTS.home}, …</button></div>
    </section>
  </div>;
}
